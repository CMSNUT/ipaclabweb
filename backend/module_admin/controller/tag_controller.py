...
from module_admin.service.tag_service import TagService
...
tagController = APIRouter(prefix='/system/tag', dependencies=[Depends(LoginService.get_current_user)])


@tagController.get(
    '/list', response_model=PageResponseModel, dependencies=[Depends(CheckUserInterfaceAuth('system:tag:list'))]
)
async def get_system_tag_list(
    request: Request,
...
@tagController.post('', dependencies=[Depends(CheckUserInterfaceAuth('system:tag:add'))])
...
@tagController.put('', dependencies=[Depends(CheckUserInterfaceAuth('system:tag:edit'))])
@ValidateFields(validate_model='edit_tag')
@Log(title='标签管理', business_type=BusinessType.UPDATE)
async def edit_system_tag(
    request: Request,
    edit_tag: TagModel,
    query_db: AsyncSession = Depends(get_db),
    current_user: CurrentUserModel = Depends(LoginService.get_current_user),
...
@tagController.delete('/{tag_ids}', dependencies=[Depends(CheckUserInterfaceAuth('system:tag:remove'))])
@Log(title='标签管理', business_type=BusinessType.DELETE)
async def delete_system_tag(request: Request, tag_ids: str, query_db: AsyncSession = Depends(get_db)):
...
@tagController.get(
    '/{tag_id}', response_model=TagModel, dependencies=[Depends(CheckUserInterfaceAuth('system:tag:query'))]
)
async def query_detail_system_tag(request: Request, tag_id: int, query_db: AsyncSession = Depends(get_db)):
...
@tagController.post('/export', dependencies=[Depends(CheckUserInterfaceAuth('system:tag:export'))])
@Log(title='标签管理', business_type=BusinessType.EXPORT)
async def export_system_tag_list(
    request: Request,
