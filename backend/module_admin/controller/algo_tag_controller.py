...
from module_admin.service.algo_tag_service import Algo_tagService
from module_admin.entity.vo.algo_tag_vo import DeleteAlgo_tagModel, Algo_tagModel, Algo_tagPageQueryModel
...
algo_tagController = APIRouter(prefix='/system/algo_tag', dependencies=[Depends(LoginService.get_current_user)])


@algo_tagController.get(
    '/list', response_model=PageResponseModel, dependencies=[Depends(CheckUserInterfaceAuth('system:algo_tag:list'))]
)
async def get_system_algo_tag_list(
    request: Request,
...
@algo_tagController.post('', dependencies=[Depends(CheckUserInterfaceAuth('system:algo_tag:add'))])
@ValidateFields(validate_model='add_algo_tag')
@Log(title='算法与标签关联', business_type=BusinessType.INSERT)
async def add_system_algo_tag(
    request: Request,
...
@algo_tagController.put('', dependencies=[Depends(CheckUserInterfaceAuth('system:algo_tag:edit'))])
@ValidateFields(validate_model='edit_algo_tag')
@Log(title='算法与标签关联', business_type=BusinessType.UPDATE)
async def edit_system_algo_tag(
    request: Request,
...
@algo_tagController.delete('/{algo_ids}', dependencies=[Depends(CheckUserInterfaceAuth('system:algo_tag:remove'))])
@Log(title='算法与标签关联', business_type=BusinessType.DELETE)
async def delete_system_algo_tag(request: Request, algo_ids: str, query_db: AsyncSession = Depends(get_db)):
...
@algo_tagController.get(
    '/{algo_id}', response_model=Algo_tagModel, dependencies=[Depends(CheckUserInterfaceAuth('system:algo_tag:query'))]
)
async def query_detail_system_algo_tag(request: Request, algo_id: int, query_db: AsyncSession = Depends(get_db)):
...
@algo_tagController.post('/export', dependencies=[Depends(CheckUserInterfaceAuth('system:algo_tag:export'))])
@Log(title='算法与标签关联', business_type=BusinessType.EXPORT)
async def export_system_algo_tag_list(
    request: Request,
