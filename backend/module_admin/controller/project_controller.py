...
projectController = APIRouter(prefix='/system/project', dependencies=[Depends(LoginService.get_current_user)])


@projectController.get(
    '/list', response_model=PageResponseModel, dependencies=[Depends(CheckUserInterfaceAuth('system:project:list'))]
)
...
project_page_query: ProjectPageQueryModel = Depends(ProjectPageQueryModel.as_query),
    ...
    # 获取分页数据
    project_page_query_result = await ProjectService.get_project_list_services(query_db, project_page_query, is_page=True)
    logger.info('获取成功')

    return ResponseUtil.success(model_content=project_page_query_result)


@projectController.post('', dependencies=[Depends(CheckUserInterfaceAuth('system:project:add'))])
...
@projectController.put('', dependencies=[Depends(CheckUserInterfaceAuth('system:project:edit'))])
...
@projectController.delete('/{project_ids}', dependencies=[Depends(CheckUserInterfaceAuth('system:project:remove'))])
@Log(title='项目管理', business_type=BusinessType.DELETE)
async def delete_system_project(request: Request, project_ids: str, query_db: AsyncSession = Depends(get_db)):
...
@projectController.get(
    '/{project_id}', response_model=ProjectModel, dependencies=[Depends(CheckUserInterfaceAuth('system:project:query'))]
)
async def query_detail_system_project(request: Request, project_id: int, query_db: AsyncSession = Depends(get_db)):
...
@projectController.post('/export', dependencies=[Depends(CheckUserInterfaceAuth('system:project:export'))])
@Log(title='项目管理', business_type=BusinessType.EXPORT)
async def export_system_project_list(
    request: Request,
