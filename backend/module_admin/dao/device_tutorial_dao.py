...
class Device_tutorialDao:
    ...
    async def get_device_tutorial_list(cls, db: AsyncSession, query_object: Device_tutorialPageQueryModel, is_page: bool = False):
        ...
        query = (
            select(SysDeviceTutorial)
            .where(
                SysDeviceTutorial.tutorial_name.like(f'%{query_object.tutorial_name}%') if query_object.tutorial_name else True,
                SysDeviceTutorial.device_id == query_object.device_id if query_object.device_id else True,
                SysDeviceTutorial.tutorial_content.like(f'%{query_object.tutorial_content}%') if query_object.tutorial_content else True,
            )
