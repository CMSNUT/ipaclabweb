...
from module_resource.service.algo_service import AlgoService
...
algoController = APIRouter(prefix='/resource/algo', dependencies=[Depends(LoginService.get_current_user)])


@algoController.get(
    '/list', response_model=PageResponseModel, dependencies=[Depends(CheckUserInterfaceAuth('resource:algo:list'))]
)
async def get_resource_algo_list(
    request: Request,
...
@algoController.get(
    '/{algo_id}', response_model=AlgoModel, dependencies=[Depends(CheckUserInterfaceAuth('resource:algo:query'))]
)
async def query_detail_resource_algo(request: Request, algo_id: int, query_db: AsyncSession = Depends(get_db)):
