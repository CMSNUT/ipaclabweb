...
from module_resource.service.device_service import DeviceService
from module_resource.entity.vo.device_vo import DeleteDeviceModel, DeviceModel, DevicePageQueryModel
...
deviceController = APIRouter(prefix='/resource/device', dependencies=[Depends(LoginService.get_current_user)])


@deviceController.get(
    '/list', response_model=PageResponseModel, dependencies=[Depends(CheckUserInterfaceAuth('resource:device:list'))]
)
async def get_resource_device_list(
    request: Request,
...
@deviceController.post('', dependencies=[Depends(CheckUserInterfaceAuth('resource:device:add'))])
@ValidateFields(validate_model='add_device')
@Log(title='仪器管理', business_type=BusinessType.INSERT)
async def add_resource_device(
    request: Request,
...
@deviceController.put('', dependencies=[Depends(CheckUserInterfaceAuth('resource:device:edit'))])
@ValidateFields(validate_model='edit_device')
@Log(title='仪器管理', business_type=BusinessType.UPDATE)
async def edit_resource_device(
    request: Request,
...
@deviceController.delete('/{device_ids}', dependencies=[Depends(CheckUserInterfaceAuth('resource:device:remove'))])
@Log(title='仪器管理', business_type=BusinessType.DELETE)
async def delete_resource_device(request: Request, device_ids: str, query_db: AsyncSession = Depends(get_db)):
...
@deviceController.get(
    '/{device_id}', response_model=DeviceModel, dependencies=[Depends(CheckUserInterfaceAuth('resource:device:query'))]
)
async def query_detail_resource_device(request: Request, device_id: int, query_db: AsyncSession = Depends(get_db)):
...
@deviceController.post('/export', dependencies=[Depends(CheckUserInterfaceAuth('resource:device:export'))])
@Log(title='仪器管理', business_type=BusinessType.EXPORT)
async def export_resource_device_list(
    request: Request,
